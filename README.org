...
|---------------------------------------+-----------------------------------------------+------------------------|
| Intent is declared, not performed     | =change:start=, the one label a person writes | §Labels                |
...
cannot. The person's two words are =change:start= and =staging:hold=;
=complete= is the pipeline's, and =end= is the pipeline's on one path only.
