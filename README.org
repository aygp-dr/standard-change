...
stateDiagram-v2
    ...
    Verified --> Held: a person adds staging:hold
    Held --> Verified: the person lifts it
    Verified --> Promoted: staging:smoke lands deploy:production (promote.yml), unless held
    Promoted --> Live: install finished -> production:deployed, then guard 5 -> production:healthy
    Live --> Complete: settle -> change:complete, merge, PIR, cleanup
    Complete --> [*]
...
cannot — and writes =change:end= as the last act. =change:end= drives
nothing. It is the tombstone that says the clearing was settlement, since a
refusal at the lock, the reaper and an eviction also leave a change with no
labels, and a reader should be able to tell those apart. The person's two
words are =change:start= and =staging:hold=; =complete= and =end= are the
pipeline's.
