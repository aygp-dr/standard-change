...
stateDiagram-v2
    ...
    Opened --> Started: a person adds change:start
    Started --> Scheduled: window reserved -> change:scheduled
    ...
    Authorized --> Complete: settle asks production what it is serving NOW -> change:complete
    Complete --> Merged: squash merge, branch deleted
    Merged --> Recorded: PIR posted, while the labels are still there to read
    Recorded --> Tidied: cleanup clears every transient label, change:complete included, berth released
    Tidied --> [*]: no tombstone on this path -- the forge's mergedAt is the record
    Forfeited --> Started: the person states the intent again

    Deployed --> Closed: abort from any state past the berth claim -> change:failed or change:backed-out
    Closed --> [*]: change:end, written last -- there is no mergedAt here, so the tombstone is the only thing saying this clearing was deliberate
...
sequenceDiagram
    ...
    alt production is serving this head
        ...
        IDP->>GH: change:complete -- validation done
        IDP->>GH: merge, squash, delete the branch
        IDP->>GH: the PIR, posted while the labels are still readable
        IDP->>GH: cleanup -- clear every transient label, change:complete included, berth released
        Note over IDP,GH: no change:end here. mergedAt is the record
        GH-->>Dev: shipped
    else it did not make it
        IDP->>GH: the closure record, then change:failed or change:backed-out
        IDP->>GH: clear every marker, close the window, release the berth
        IDP->>GH: change:end, last -- nothing else says this clearing was deliberate
    end
#+end_src

** One label starts a change

A person adds *=change:start=* and that is the whole of their part — one
label, not two: it supersedes =change:requested= plus =release=, which were
two human-owned labels for one fact. Everything after is the system acting or
observing, and the *name shape* says which:

| shape                | kind        | example               | added by hand? |
|----------------------+-------------+-----------------------+----------------|
| =change:start=       | intent      | someone said go       | *yes*          |
...
1. *re-measures production against the head, now.* Not /was healthy once/: it
   ...
   *not* required here.
2. *sets =change:complete=* — validation is done. Before the merge, because
...
5. *cleans up* — clears every transient label, =change:complete= included, and
   with it the berth.
...
marker cleared, the closure code (=change:failed= or =change:backed-out=), the
berth released — and then =change:end=, last.
...
=itil:standard= plus =change:complete= was a legitimate pair:

| namespace  | kind             | members                                              | cardinality |
|------------+------------------+------------------------------------------------------+-------------|
| =itil:*=   | *classification* | =itil:standard= · =itil:normal= · =itil:emergency=   | exactly one |
| =change:*= | *lifecycle*      | =change:start= → =change:scheduled= → =change:complete= | one active |
...
person's =change:start=, a reviewer's approval, green gates on the head,
...
=change:complete= is the pipeline's *outcome*: set when every guard has passed
...
cannot. The person's three words are =change:start=, =staging:hold= and
=release:unaffected= (nothing to release: the only act left is the merge, and
only if the labeller attached no =app:*=); =complete= is the pipeline's, and
=end= is the pipeline's on one path only.
...
=change:complete=, and it condemns =change:end= on the same path for the same
...
The evidence is not hypothetical. PR #52 carried =change:end= through an
...
every label, then adding =change:end= to say the clearing happened, then
...
| *aborted* (closed unmerged) | =change:failed= / =change:backed-out=, by =change/abort.sh= | *=change:end=, last* |

An aborted change has no =mergedAt=. A refusal at the lock, the reaper and an
eviction all leave a change with no labels too, so on that path =change:end=
...
the script: =change/settle.sh= still adds =change:end= after the clearing, and
...
| a change with no lifecycle label and a staging verdict still on it | the reaper: it removes =change:scheduled= and =deploy:staging= and leaves the observations | documented; the observations are about a head that has not changed, on an environment that is gone |

A fourth — =change:failed= beside =deploy:production= — was reachable because
...
|----------------+---------------------------+--------------------------------|
| =change:start= | *a person only* — intent  | *automation* — the driver consumes it when it claims the berth |
...
A person must add =change:start= because it is the intent to ship and no machine
...
Decided 2026-09-14. =deploy:staging= is the environment lock: one change
...
lifecycle, the observations, and the human intent (=change:start=,
=change:requested=, =release=, =staging:hold=) — and leaves one comment
naming the holder. The person re-states the intent when the lock is free.
=blocked:queue= is retired: a label left behind by a refusal is a print
statement that later reads as a claim. Rule =LockResets= in =tla/Labels.tla=;
the negative run shows the refused change keeping its markers in six states.

*** =staging:hold= — the one human stop, re-introduced at one scope

=hold:staging= was rejected on 2026-09-13 because it was a third way to stop
a deploy at a third scope with no rule for which applied. =staging:hold= is
the same human intent — /I want to look before this goes to production/ —
...
(=research/history/precedence-and-locks.org=). It was a per-change hold and the only human
...
|---------------------------+--------------------------------+-------|
| it should not go at all   | *do not add =change:start=*     | that change |
...
flowchart TB
    ...
    E1 -.->|"cannot promote"| X(("✗"))
    E2 -.->|"cannot promote"| X
    W1 -->|"change:start<br/>guards 0 and 1"| S
