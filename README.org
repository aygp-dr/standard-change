...
because =deploy-staging= died at =queue.sh= before deploying (issue #25). PRs
read "1 failed deployment" while the estate served that build perfectly: the
record described the *workflow run*, not the environment. That workflow has
since been removed — it never once deployed anything, in 249 runs — and the
forge no longer deploys at all (=docs/adr/0004-the-forge-does-not-deploy.org=).
...
=docs/adr/0003= records why: six of seven guards do not read the calendar at
