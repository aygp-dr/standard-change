...
stateDiagram-v2
    direction TB
    [*] --> Submitted: PR opened
    Submitted --> Assessing: labeller sets app:*, change:standard
...
sequenceDiagram
    ...
    Dev->>GH: push -> PR
    GH->>GH: labeller: app:*, change:standard
...
=change:requested= → =change:scheduled= → *=change:complete=*. That is the state
machine, and =change:complete= is the *terminal state*.
...
*calendar event* blocks the estate, while a =change:emergency= *label* exempts
