...
test('render identifies this app, not another', () => {
  assert.equal(render('/x').app, 'checkout');
  assert.equal(meta.app, 'checkout');
});
...
const EXPECTED_ROUTES = ["/checkout", "/checkout/payment", "/checkout/confirm"].map(String);
...
test('every declared route is servable', () => {
  for (const r of meta.routes) {
    ...
    assert.equal(render(probe).app, 'checkout', `route ${r} not served`);
  }
});
