...
# Both are read by gates/oracle.sh, which is sourced below. shellcheck runs
# each script in isolation (gates/shellcheck.sh has no -x), so it cannot see
# the use and reports SC2034; the directive carries the reason rather than the
# gate carrying a warning tier, which this repo does not have.
# shellcheck disable=SC2034  # read by gates/oracle.sh, sourced on the next line
ORACLE_LIB=1
# shellcheck disable=SC2034  # ditto: oracle.sh must not derive the root from cwd
ORACLE_ROOT="$PWD"
. ./gates/oracle.sh
